package com.husky.controller.system.head;
...
import com.husky.service.system.appuser.AppuserService;
import com.husky.service.system.user.UserService;
import com.husky.util.AppUtil;
import com.husky.util.Const;
import com.husky.util.PageData;
import com.husky.util.SmsUtil;
import com.husky.util.Tools;
import com.husky.util.Watermark;
import com.husky.util.mail.SimpleMailSender;
...
public class HeadController extends BaseController {
	...
	public Object sendEmail(){
		...
		if(null != strEMAIL && !"".equals(strEMAIL)){
			String strEM[] = strEMAIL.split(",husky,");
			if(strEM.length == 4){
				if("yes".endsWith(isAll)){
					try {
						List<PageData> userList = new ArrayList<PageData>();
	...
	public ModelAndView goEditEmail() throws Exception{
		...
		if(null != strSMS1 && !"".equals(strSMS1)){
			String strS1[] = strSMS1.split(",husky,");
			if(strS1.length == 2){
		...
		if(null != strSMS2 && !"".equals(strSMS2)){
			String strS2[] = strSMS2.split(",husky,");
			if(strS2.length == 2){
		...
		if(null != strFWATERM && !"".equals(strFWATERM)){
			String strFW[] = strFWATERM.split(",husky,");
			if(strFW.length == 5){
		...
		if(null != strIWATERM && !"".equals(strIWATERM)){
			String strIW[] = strIWATERM.split(",husky,");
			if(strIW.length == 4){
	...
	public ModelAndView saveSys() throws Exception{
		...
		Tools.writeFile(Const.EMAIL,pd.getString("SMTP")+",husky,"+pd.getString("PORT")+",husky,"+pd.getString("EMAIL")+",husky,"+pd.getString("PAW"));	//写入邮件服务器配置
		Tools.writeFile(Const.SMS1,pd.getString("SMSU1")+",husky,"+pd.getString("SMSPAW1"));	//写入短信1配置
		Tools.writeFile(Const.SMS2,pd.getString("SMSU2")+",husky,"+pd.getString("SMSPAW2"));	//写入短信2配置
		mv.addObject("msg","OK");
	...
	public ModelAndView saveSys2() throws Exception{
		...
		PageData pd = new PageData();
		pd = this.getPageData();
		Tools.writeFile(Const.FWATERM,pd.getString("isCheck1")+",husky,"+pd.getString("fcontent")+",husky,"+pd.getString("fontSize")+",husky,"+pd.getString("fontX")+",husky,"+pd.getString("fontY"));	//文字水印配置
		Tools.writeFile(Const.IWATERM,pd.getString("isCheck2")+",husky,"+pd.getString("imgUrl")+",husky,"+pd.getString("imgX")+",husky,"+pd.getString("imgY"));	//图片水印配置
		Watermark.fushValue();
		mv.addObject("msg","OK");
		mv.setViewName("save_result");
		return mv;
