package com.husky.plugin.websocketInstantMsg;
...
public class ChatServer extends WebSocketServer{
	...
	public void onMessage(WebSocket conn, String message){
		...
		if(null != message && message.startsWith("huskyadminqq313596790")){
			this.userjoin(message.replaceFirst("huskyadminqq313596790", ""),conn);
		}if(null != message && message.startsWith("Leavehuskyadminqq313596790")){
			this.userLeave(conn);
		}if(null != message && message.contains("huskyadmin886")){
			String toUser = message.substring(message.indexOf("huskyadmin886")+10, message.indexOf("huskyhuskyadmin888"));
			message = message.substring(0, message.indexOf("huskyadmin886")) +"[私信]  "+ message.substring(message.indexOf("huskyhuskyadmin888")+12, message.length());
